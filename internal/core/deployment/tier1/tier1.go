...
func DeployTier1(ctx context.Context, cfg *config.Config) error {
	...
	// Step 3-10: Deploy components
	deployFuncs := []struct {
		name string
		fn   func(context.Context, *config.Config, *ui.StepProgress, int) error
	}{
	...
	for i, deploy := range deployFuncs {
		stepIdx := currentStep + i
		if err := deploy.fn(ctx, cfg, progress, stepIdx); err != nil {
			progress.FailStep(stepIdx, err)
			return fmt.Errorf("failed to deploy %s: %w", deploy.name, err)
		}
	}
...
// deployWithProgress is a generic helper that deploys a Helm chart with progress tracking.
// It handles the common pattern of: StartStep -> DeployHelmChart -> CompleteStep/FailStep
...
func deployWithProgress(ctx context.Context, progress *ui.StepProgress, step int, opts shared.HelmDeploymentOptions, dynamicValuesFn func(context.Context) (map[string]interface{}, error)) error {
	...
	// If dynamic values function is provided, call it and merge the results
...
func deployCilium(ctx context.Context, cfg *config.Config, progress *ui.StepProgress, step int) error {
...
func deployLocalPathStorage(ctx context.Context, cfg *config.Config, progress *ui.StepProgress, step int) error {
	progress.StartStep(step)
...
func deployFalco(ctx context.Context, cfg *config.Config, progress *ui.StepProgress, step int) error {
	return deployWithProgress(ctx, progress, step, shared.HelmDeploymentOptions{
		ReleaseName:     "falco",
...
func deployGPUOperator(ctx context.Context, cfg *config.Config, progress *ui.StepProgress, step int) error {
...
func deployCertManager(ctx context.Context, cfg *config.Config, progress *ui.StepProgress, step int) error {
	progress.StartStep(step)
...
func deployTrustManager(ctx context.Context, cfg *config.Config, progress *ui.StepProgress, step int) error {
	progress.StartStep(step)
	...
	ui.Info("Labeling kube-system namespace for CA injection...")
...
func deployEnvoyAIGateway(ctx context.Context, cfg *config.Config, progress *ui.StepProgress, step int) error {
	progress.StartStep(step)

	// Create namespace
	if err := k8s.CreateNamespace(ctx, "envoy-ai-gateway-system"); err != nil {
		progress.FailStep(step, fmt.Errorf("failed to create namespace: %w", err))
	...
	if err := k8s.ApplyURL(ctx, constants.ManifestGatewayAPIInferenceExtension); err != nil {
		progress.FailStep(step, fmt.Errorf("failed to install inference extension CRDs: %w", err))
...
func deployEnvoyGateway(ctx context.Context, cfg *config.Config, progress *ui.StepProgress, step int) error {
	progress.StartStep(step)
	...
	if err != nil {
		progress.FailStep(step, fmt.Errorf("failed to generate Redis password: %w", err))
	...
	if err := k8s.CreateSecret(ctx, "envoy-gateway-system", "redis", map[string]string{
		...
		progress.FailStep(step, fmt.Errorf("failed to create Redis secret: %w", err))
